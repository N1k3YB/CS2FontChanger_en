...
class LoadingScreen(QWidget):
    ...
    def setup_ui(self):
        ...
        # Loading text
        self.loading_label = QLabel("Search CS2...")
...
class MainWindow(QMainWindow):
    ...
    def setup_ui(self):
        ...
        # Buttons
        self.select_path_btn = StyledButton("Specify path to CS2")
        ...
        self.font_btn = StyledButton("Choose .ttf/.otf file")
        ...
        self.font_label = QLabel("The font is not selected")
        ...
        self.apply_btn = StyledButton("Apply Font")
    ...
    def on_search_complete(self, path):
        ...
        if path:
            ...
            self.status_label.setText(f"CS2 found: {path}")
            self.select_path_btn.hide()
            self.font_btn.setEnabled(True)
        else:
            self.status_label.setText("CS2 not found")

    def select_steam_path(self):
        path = QFileDialog.getExistingDirectory(self, "Select folder with CS2")
        if path:
            if self.validate_csgo_path(path):
                ...
                self.show_message("Error", "There is no CS2 in this folder")
    ...
    def select_font(self):
        font_path, _ = QFileDialog.getOpenFileName(
            self, "Choose a font", "", "Font files (*.ttf *.otf)")
        if font_path:
            self.selected_font = font_path
            self.font_label.setText(f"Selected font: {os.path.basename(font_path)}")
        ...
        try:
            ...
            if os.path.exists(target_font_path):
                try:
                    ...
                    raise Exception(f"Couldn't delete an existing font file: {str(e)}")
                    
            shutil.copy2(self.selected_font, fonts_dir)
            
            self.show_message("Success", "The font has been successfully changed!")
            
        except Exception as e:
            self.show_message("Error", f"You can't use the font from this folder\nMoving it to another folder")
            print(e)
    ...
    def create_config_file(self, target_path, template, font_name, font_file):
        ...
        try:
            ...
            raise Exception(f"Error creating the configuration file: {e}")
